...
```{r}
hotel.log<- myhotel %>% 
  mutate(above_avg = ifelse(avg_price_per_room >mean(avg_price_per_room), 1,0)) %>% 
  dplyr::select(-avg_price_per_room)
```


```{r}
logit_mod <- glm(above_avg~., hotel.log, family = "binomial")
```
...
```{r}
hotel.log2<- hotel.log %>% 
  select(-babies,-country,-previous_cancellations,-previous_bookings_not_canceled,-market_segment, -company, -days_in_waiting_list, -stays_in_week_nights, -booking_changes)
```


```{r}
logit_mod2 <- glm(above_avg~., hotel.log2, family = "binomial")
```
